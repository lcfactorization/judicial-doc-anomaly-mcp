...
description: |
  ...
  在原有12维检测框架基础上，引入时间一致性检测、语义漂移检测、缺失信息分析（负空间检测）、类案偏离量化、程序行为链建模与多Agent对抗校验机制，扩展为16维度检测体系。20260506版重大更新：维度三"事实认定与关键情节记录检测"全面升级为26项精细化检测体系（F-01至F-26），覆盖证据支撑缺陷、证据采信偏差、证据不当排除、逻辑说理错误、举证责任分配五大子类，并新增"事实认定错误快速对照清单（翻案速查表）"，实现事实认定错误的系统化、标准化、可量化检测。维度从15维扩展为16维（新增维度十五·类案偏离量化检测），维度顺序调整为"形式→内容→综合"五层逻辑结构。旨在识别司法/行政文书中的结构性偏差、潜在不公及异常裁判路径，提供高鲁棒性与抗规避能力的审查分析。
license: MIT
---
...
按照本技能定义的十六大维度，将案卷材料与 Phase 3 建立的"基准"进行逐条比对，记录所有检测到的异常点，精准定位原文出处（页码/段落/行号/证据编号）。

**维度逻辑层次**：
- **第一层·形式审查**（维度一至二）：程序操作→证据审查标准
- **第二层·内容审查**（维度三至六）：事实认定→争议焦点→法律适用→自由裁量
- **第三层·说理与逻辑**（维度七至八）：文书表述→逻辑链条
- **第四层·时间与过程**（维度九至十）：时间一致性→庭审行为
- **第五层·综合评判**（维度十一至十六）：外部干预→执行→缺失信息→语义漂移→类案偏离→异常耦合
...
> [!NOTE]
> **与其他维度的交叉索引**：
> - F-07至F-13（证据采信偏差）与 **维度二：证据采信与审查标准** 深度交叉，建议联合审查
> - F-17（未经质证）同时触发 **维度一：程序操作与正当性**
> - F-22（回避争议焦点）同时触发 **维度四：争议焦点偏移与遗漏**
> - F-24/F-25（说理矛盾/模板套话）同时触发 **维度七：文书表述与说理断裂**
> - F-20/F-23（事实法律不对应/逻辑断裂）同时触发 **维度八：逻辑闭环与推理链条**
> - F-05（时间线混乱）同时触发 **维度九：时间一致性与期限合规**
> - F-21（因果关系颠倒）同时触发 **维度十四：语义漂移与概念偷换**
> - F-14/F-19（证据不提/遗漏）同时触发 **维度十三：缺失信息与负空间分析**
...
> [!NOTE]
> **交叉索引**：本维度与 **维度三·子类D·F-24（前后说理自相矛盾）** 及 **F-25（照搬模板套话）** 直接对应。文书表述的模板化与逻辑不自洽，是事实认定缺乏实质审理的外在表征。建议将维度七与维度三·子类D联合审查。
...
> [!NOTE]
> **交叉索引**：本维度与 **维度三·子类D·F-20（事实与法律不对应）** 及 **F-23（逻辑断裂）** 直接对应。逻辑链条检测是事实认定错误的"结构层面"验证——当证据→事实→法律→结论任一环节断裂，必然导致事实认定错误。建议将维度八与维度三·子类D联合审查。
...
> [!NOTE]
> **交叉索引**：本维度与 **维度三·子类A·F-05（时间线认定混乱错误）** 直接对应。时间一致性是事实认定的"硬约束"——时间不会说谎，时间线上的矛盾是最难被反驳的事实认定错误类型。建议将维度九与F-05联合审查。
...
| 检测项        | 判定逻辑                                                     | 依据材料                               |
| :------------ | :----------------------------------------------------------- | :------------------------------------- |
| 地方保护/行政干预迹象  | 本地龙头企业/纳税大户/行政机关作为被告时的异常胜诉率；涉及外地当事人的案件裁判尺度存在差异；行政/监管部门对案件有隐性影响 | 用户提供的背景信息 + 公开案例检索      |
...
> [!NOTE]
> **交叉索引**：本维度与 **维度三·子类C·F-14（合法证据只字不提）** 及 **F-19（遗漏重要证据或关键事实）** 直接对应。负空间检测是发现"沉默型"事实认定错误的核心工具——法院通过"不说"来规避对不利事实的认定，这种沉默本身就是最隐蔽的异常信号。建议将维度十三与维度三·子类C联合审查。
...
**检测逻辑**：将本案裁判结果与同省/直辖市近5年同类案件的裁判结果分布进行统计比对，量化偏离程度，识别是否存在超出正常裁量范围的异常裁判。

| 检测项         | 判定逻辑                                                     | 依据材料                               |
| :------------- | :----------------------------------------------------------- | :------------------------------------- |
| 赔偿金额偏离   | 本案判赔金额与类案均值偏离超过1个标准差                      | 类案检索结果 + 本案裁判结果            |
| 责任比例偏离   | 本案责任划分比例与类案均值偏离超过1个标准差                  | 类案检索结果 + 本案裁判结果            |
| 支持率偏离     | 本案对某类主张的支持/驳回比例与类案分布显著不同              | 类案检索结果 + 本案裁判结果            |
| 定性结果偏离   | 本案对法律关系的定性（如劳动关系vs劳务关系）与类案主流定性不一致 | 类案检索结果 + 本案法律适用            |
...
> [!NOTE]
> **交叉索引**：本维度与 **维度六（自由裁量权行使）** 及 **维度十六（异常耦合）** 直接对应。类案偏离是自由裁量权滥用的量化证据——当偏离度超过2个标准差且与其他维度异常耦合时，构成结构性偏差的强信号。建议将维度十五与维度六、维度十六联合审查。

### 维度十六：异常耦合与结构性偏差

**检测逻辑**：单个维度的异常可能源于业务水平或偶然因素，但当**多个维度的异常同步出现，且所有异常指向均对同一方当事人有利**时，提示存在**结构性偏差**。

**触发条件**：在以下维度类别中，至少**三个类别**各触发**1项**高置信异常，且全部指向对特定一方有利：
- **程序类**（维度一、十）
- **证据类**（维度二、八）
- **事实类**（维度三、四）
- **法律类**（维度五、六）
...
5. **联合维度**：结合维度二（证据采信）、维度四（争议焦点）、维度八（逻辑链条）进行交叉验证
...
| **Analyzer Agent**         | 执行十六维度扫描，生成初步异常清单                           |
...
`{Agent名称}_{大模型及版本号}_司法行政文书异常检测报告_{案件简称}_v{版本号}_{YYYYMMDD}.md`

示例：`QClaw_GLM-5.1_司法行政文书异常检测报告_张某劳动争议案_v0.5.0_20260421.md`
...
> [!NOTE]
> **基础信息档案**
> - **报告编号**：JL-[YYYYMMDDHHMM]
> - **案件名称/案号**：[提取案号]
> - **文书类型**：[判决书/裁决书/决定书等]
> - **分析模型**：[AI Agent名/大模型及版本号]
> - **检测时间**：[YYYY-MM-DD]
> - **材料完整性评分**：[XX/100]
> - **检测引擎**：司法文书异常检测引擎 v[版本号]
...
>
> 1. 结合外部背景信息（如人际网络、地方保护/行政干预因素）进行更高层级审查
...
## 十三、体制与人情洞察（Institutional Dynamics Awareness）
...
## 十四、报告格式规范（Report Format Standards）

> [!IMPORTANT]
> 以下格式规范适用于所有提交法院或合议庭的正式报告，必须严格遵守。

### 14.1 总结/归纳/点评：优先使用表格
...
当报告需要提交合议庭时，合议庭称呼必须使用HTML代码确保**0缩进**（Typora可能设置默认段落缩进，必须用内联样式覆盖）：
...
签名档优先使用指定模板（`证据材料/此致[匿名化市]中级人民法院签名档.md`），日期调整为当前日期。
