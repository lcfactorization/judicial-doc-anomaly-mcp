"""Configuration management for judicial-lint-mcp v0.2.0"""
...
class DetectionConfig(BaseModel):
    ...
    enable_graph_building: bool = Field(default=True, description="Enable graph structure modeling")
    enable_quality_assessment: bool = Field(default=True, description="Enable 7-dimension quality assessment")
    enable_quick_check: bool = Field(default=True, description="Enable 26-item quick check table")
...
class AdversarialConfig(BaseModel):
    ...
    enable_devils_advocate: bool = Field(default=True, description="Enable Devil's Advocate Q1/Q2/Q3")
    enable_multi_role: bool = Field(default=True, description="Enable 5-role adversarial review")
    enable_cross_examination: bool = Field(default=True, description="Enable cross-examination between roles")


class GraphConfig(BaseModel):
    enable_evidence_graph: bool = Field(default=True, description="Build Evidence Graph")
    enable_procedure_graph: bool = Field(default=True, description="Build Procedure Graph")
    enable_reasoning_graph: bool = Field(default=True, description="Build Legal Reasoning Graph")
    output_mermaid: bool = Field(default=True, description="Output Mermaid diagram format")
    ...
    def from_env(cls) -> "AppConfig":
        ...
        return cls(
            ...
            detection=DetectionConfig(
                ...
                enable_graph_building=os.getenv("ENABLE_GRAPH_BUILDING", "true").lower() == "true",
                enable_quality_assessment=os.getenv("ENABLE_QUALITY_ASSESSMENT", "true").lower() == "true",
                enable_quick_check=os.getenv("ENABLE_QUICK_CHECK", "true").lower() == "true",
            ),
